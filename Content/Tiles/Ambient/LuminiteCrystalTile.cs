...
{
    public abstract class LuminiteCrystalTile : ModTile
    {
        ...
        public override void ModifyLight(int i, int j, ref float r, ref float g, ref float b)
        {
            ...
            r = (float)Math.Abs(Math.Cos(((i*j)+Main.time))*0.07f)*0.3f;
            g = ((float)Math.Abs(Math.Cos(((i)+Main.time))*0.12f)*0.5f)+0.5f;
            b = ((float)Math.Abs(Math.Cos(((j)+Main.time))*0.03f)*0.3f)+0.2f;
        }
